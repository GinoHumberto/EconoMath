...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[>=Stealth, scale=1.1]
        ...
        % Eje vertical z a la derecha
        \draw (7,0) -- (7,5) node[right] {$z$};

        % Proyecciones y punto (x1, y1)
        \draw[dashed] (1.5,0) -- (1.5,3) -- (0,3);
        \fill (1.5,3) circle (1.5pt) node[right, xshift=2pt] {$(x_1, y_1)$};
        \node[below] at (1.5,0) {$x_1$};
        \node[left] at (0,3) {$y_1$};

        % Proyecciones y punto (x2, y2)
        \draw[dashed] (3,0) -- (3,2) -- (0,2);
        \fill (3,2) circle (1.5pt) node[right, xshift=2pt] {$(x_2, y_2)$};
        \node[below] at (3,0) {$x_2$};
        \node[left] at (0,2) {$y_2$};

        % Puntos en el eje z
        \fill (7,3.5) circle (1.5pt) node[right] {$z_1$};
        \fill (7,2.2) circle (1.5pt) node[right] {$z_2$};

        % Flechas curvas de la función g
        \draw[->, thick, color=blue] (1.5,3) to[out=40, in=160] node[above] {$g$} (7,3.5);
        \draw[->, thick, color=blue] (3,2) to[out=30, in=170] node[above] {$g$} (7,2.2);

    \end{tikzpicture}
    \caption{Representación de la aplicación $g$ desde el plano $xy$ hacia el eje $z$.}
    \label{grafico_tidimensional_1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        >=Stealth, 
        scale=0.8,
        x={(-0.8cm,-0.4cm)}, % Eje x: hacia la izquierda y abajo
        y={(0.8cm,-0.4cm)},  % Eje y: hacia la derecha y abajo
        z={(0cm,1.2cm)}      % Eje z: vertical hacia arriba
    ]
        % --- EJES ---
        \draw[->] (0,0,0) -- (6,0,0) node[below left] {$x$};
        \draw[->] (0,0,0) -- (0,7,0) node[below right] {$y$};
        \draw[->] (0,0,0) -- (0,0,2.5) node[left] {$z$}; % Eje z más pequeño

        % --- PUNTO 1 (x1, y1, z1) ---
        % Proyecciones en el plano xy 
        \draw[dashed] (2.5,0,0) -- (2.5,4,0) -- (0,4,0);
        
        % Línea de altura vertical z1 - Curva de color azul
        \draw[dashed] (2.5,4,0) -- (2.5,4,2); % Conexión base-tope ajustada
        \draw[ultra thick, color=blue] (2.5,4,0) -- (2.5,4,2); % Línea azul
        
        % Puntos
        \fill (2.5,4,0) circle (1.5pt); 
        \fill (2.5,4,2) circle (1.5pt); % node[above] {$(x_1, y_1, z_1)$}
        
        % Etiquetas en los ejes
        \node[below left] at (1.8,0,0.4) {$x_1$};
        \node[below right] at (0,4,0.4) {$y_1$};

        % --- PUNTO 2 (x2, y2, z2) ---
        \draw[dashed] (5,0,0) -- (5,2,0) -- (0,2,0);
        
        % Línea de altura vertical z2 - Curva de color azul
        \draw[dashed] (5,2,0) -- (5,2,1.2); % Conexión base-tope ajustada
        \draw[ultra thick, color=blue] (5,2,0) -- (5,2,1.6); % Línea azul
        
        % Puntos
        \fill (5,2,0) circle (1.5pt);
        \fill (5,2,1.6) circle (1.5pt); %node[above] {$(x_2, y_2, z_2)$}
        
        % Etiquetas en los ejes
        \node[below left] at (4.6,0,0.4) {$x_2$};
        \node[below right] at (0,2,0.4) {$y_2$};
    \end{tikzpicture}
    \caption{Gráfico tridimensional.}
    \label{grafico_tridimensional_2}
\end{figure}
...
\begin{nota}
    En lugar de denotar las variables independientes mediante \(x,u,v,w, \dots\), se intercambiaron por \(x_{subindice}\), es decir \(x_1, x_2, \dots, x_n\).
